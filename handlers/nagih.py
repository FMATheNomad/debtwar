...
from services.game_logic import execute_nagih
from anti_abuse.abuse_service import check_rate_limit, check_bankruptcy_block

logger = logging.getLogger(__name__)


async def cmd_nagih(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    remaining = await check_cooldown(lender.id, "nagih")
    if remaining > 0:
    ...
    if not context.args:
        await update.message.reply_text(t("invalid_format_nagih", lang))
        return

    target_name = parse_mention(context.args[0])

    if target_name.lower() == lender_uname.lower():
        await update.message.reply_text(t("self_nagih", lang))
        return

    result = await execute_nagih(lender.id, lender.first_name, target_name, lang, context)
